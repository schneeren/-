...
-(void)touchZhuzhuang:(UIButton *)button{
    ...
    if (_selectTag>0) {
        UIButton *button = [self viewWithTag:_selectTag];
        button.backgroundColor = [self swichColorWithNumb:(int)(_selectTag-200000)];
    }

    if (_selectColor) {
        button.backgroundColor = _selectColor;
...
-(UIColor *)swichColorWithNumb:(int )num{
    ...
    for (int i = 0; i<_dataSource.count; i++) {
        [array addObject:[self loadRandomColor]];
    }
    _colorArry = [NSArray arrayWithArray:array];
    if (num<_colorArry.count) {
        return _colorArry[num];
    }
